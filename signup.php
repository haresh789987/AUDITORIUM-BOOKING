...
if(isset($_POST['signup']))
  {
    ...
    if ($query->rowCount() == 0) {
        $sql = "INSERT INTO tbluser (FullName, MobileNumber, Email, Password, Department) VALUES (:fname, :mobno, :email, :password, :department)";
        $query = $dbh->prepare($sql);
        $query->bindParam(':fname', $fname, PDO::PARAM_STR);
        $query->bindParam(':email', $email, PDO::PARAM_STR);
        $query->bindParam(':mobno', $mobno, PDO::PARAM_INT);
        $query->bindParam(':password', $password, PDO::PARAM_STR);
        $query->bindParam(':department', $department, PDO::PARAM_STR); // Add this line
        $query->execute();
...
<div class="col-md-6 contact-form-right">
    ...
    <div class="agileinfo-contact-form-grid">
        <form method="post" name="signup" onsubmit="return checkpass();">
            ...
            <br>
           <select name="department" required="true" style="display: inline-block; background: none; border: 1.5px solid #BBBBBB; width: 100%; outline: none; padding: 10px 15px 10px 15px; font-size: .9em; color: black; margin-bottom: 1.5em;">
    <option value="" disabled selected>Select Department</option>
    <?php
    // Fetch department names from the database
    $deptQuery = $dbh->query("SELECT department FROM tbldept");
